...
| Feature | How |
|---|---|
| **Sessions in separate windows** | `Shift`+click a device to open it in its own window · right-click a session tab → *Move to its own window* / *Move into the main session window* · **drag a tab to reorder it, out to tear it into its own window, onto another window's tab strip to move it there, or drag a whole window onto another's strip to combine them** · setting *Open sessions in: Tabs / A separate window per session* |
