...
| Feature | How |
|---|---|
| **Sessions in separate windows** | `Shift`+click a device to open it in its own window · right-click a session tab → *Move to its own window* / *Move into the main session window* · **drag a tab out to tear it into its own window, drag it onto another window's tab strip to move it there, or drag a whole session window onto another's tab strip to combine them** · setting *Open sessions in: Tabs / A separate window per session* |
