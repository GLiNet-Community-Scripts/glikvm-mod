...
export const MOD_VERSION = "0.1.6";
    ...
    },
    {
      ...
      apply: (src) =>
        replaceOnce(
          ...
          [
            "    // glikvm-mod ---------------------------------------------------------\n",
            "    // force the same clean re-render a manual tab switch does, to clear any stale panel\n",
            "    const glRepaint = () => {\n",
            "      const cur = currentViewDeviceId.value;\n",
            "      if (cur == null) return;\n",
            "      currentViewDeviceId.value = null;\n",
            "      requestAnimationFrame(() => requestAnimationFrame(() => { currentViewDeviceId.value = cur; }));\n",
            "    };\n",
            '    window.utils.glOn("glRepaint", () => glRepaint());\n',
            ...
            "      { immediate: true }\n",
            "    );\n",
            "    // drag a tab: tear it out to its own window, or drop it on another window's tab strip\n",
            '    const glStyle = document.createElement("style");\n',
            "    glStyle.textContent =\n",
            '      "[data-gl-device-id]{cursor:grab}" +\n',
            '      ".gl-dragging [data-gl-device-id]{cursor:grabbing}" +\n',
            '      ".gl-tab-dragging{opacity:.45;transform:scale(.96);transition:opacity .12s ease,transform .12s ease}" +\n',
            '      ".gl-drop-target .remote-header{position:relative}" +\n',
            "      \".gl-drop-target .remote-header::after{content:'';position:absolute;inset:0;pointer-events:none;border:2px solid var(--gl-color-brand-primary);border-radius:4px;background:color-mix(in srgb,var(--gl-color-brand-primary) 14%,transparent);transition:opacity .12s ease}\";\n",
            "    document.head.appendChild(glStyle);\n",
            "    let glDrag = null;\n",
            "    let glSwallowClick = false;\n",
            "    let glOverSent = 0;\n",
            "    const glClearDrag = () => {\n",
            "      if (glDrag && glDrag.el) glDrag.el.classList.remove('gl-tab-dragging');\n",
            ...
            "        if (!el) return;\n",
            '        if (e.target.closest && e.target.closest(".gl-icon-close-win")) return;\n',
            '        const id = el.getAttribute("data-gl-device-id");\n',
            "        const cfg = configList.value.find((c) => c.device.id === id);\n",
            "        if (!cfg) return;\n",
            "        glDrag = { id, el, name: cfg.device.deviceName, x0: e.screenX, y0: e.screenY, moved: false };\n",
            "      },\n",
            "      true\n",
            "    );\n",
            "    const glReorder = (clientX) => {\n",
            '      const tabs = [...document.querySelectorAll("[data-gl-device-id]")];\n',
            "      if (tabs.length < 2) return;\n",
            "      let toIdx = tabs.length;\n",
            "      for (let i = 0; i < tabs.length; i++) {\n",
            "        const r = tabs[i].getBoundingClientRect();\n",
            "        if (clientX < r.left + r.width / 2) { toIdx = i; break; }\n",
            "      }\n",
            "      const list = configList.value;\n",
            "      const fromIdx = list.findIndex((c) => c.device.id === glDrag.id);\n",
            "      if (fromIdx === -1) return;\n",
            ...
            "        if (!glDrag) return;\n",
            "        if (!glDrag.moved) {\n",
            "          if (Math.hypot(e.screenX - glDrag.x0, e.screenY - glDrag.y0) <= 10) return;\n",
            "          glDrag.moved = true;\n",
            '          document.body.classList.add("gl-dragging");\n',
            "          if (glDrag.el) glDrag.el.classList.add('gl-tab-dragging');\n",
            "        }\n",
            "        if (e.clientY <= 46) glReorder(e.clientX);\n",
            "        const now = Date.now();\n",
            "        if (now - glOverSent > 40) {\n",
            "          glOverSent = now;\n",
            "          window.utils.glTabDragOver({ x: e.screenX, y: e.screenY, name: glDrag.name });\n",
            "        }\n",
            "      },\n",
            "      true\n",
            "    );\n",
            ...
            "        e.stopPropagation();\n",
            "        glSwallowClick = true;\n",
            "        window.utils.glTabDragEnd({ deviceId: d.id, x: e.screenX, y: e.screenY });\n",
            "      },\n",
            "      true\n",
            "    );\n",
            "    document.addEventListener(\n",
            ...
            "      true\n",
            "    );\n",
            '    window.utils.glOn("glDragHighlight", (_e, on) => document.body.classList.toggle("gl-drop-target", !!on));\n',
            "    document.addEventListener(\n",
            '      "contextmenu",\n',
            "      (e) => {\n",
            '        const el = e.target && e.target.closest ? e.target.closest("[data-gl-device-id]") : null;\n',
